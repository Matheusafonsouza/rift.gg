        ...
        {match.isLive ? (
          <div>
            <div className="flex items-center justify-end gap-1.5 mb-0.5">
              ...
              <span className="font-cond text-[10px] font-bold tracking-widest uppercase text-crimson">Live</span>
            </div>
            <div className="font-cond text-sm font-bold tabular-nums">
              ...
              <span className="text-text-muted">{match.team2.gameWins ?? 0}</span>
            </div>
          </div>
        ) : match.state === "completed" ? (
          <div>
            <div className="font-cond text-xs text-text-muted mb-0.5">Final</div>
            <div className="font-cond text-sm font-bold tabular-nums">
              <span className={match.team1.outcome === "win" ? "text-emerald" : "text-text-muted"}>{match.team1.gameWins ?? 0}</span>
              <span className="text-text-faint mx-1">–</span>
              <span className={match.team2.outcome === "win" ? "text-emerald" : "text-text-muted"}>{match.team2.gameWins ?? 0}</span>
            </div>
          </div>
        ) : (
          <div>
            <div className="match-time">{match.relativeTime}</div>
            <div className="font-cond text-[10px] text-text-muted mt-0.5">Bo{match.bestOf}</div>
          </div>
        )}
      </div>
...
function EventRow({ event }: { event: typeof EVENTS[number] }) {
  ...
  return (
    <div className="hover-row group cursor-pointer">
      <div className="w-0.5 h-9 rounded-full shrink-0" style={{ backgroundColor: event.color, opacity: isUpcoming ? 0.35 : 0.7 }} />
      <div className="flex-1 min-w-0">
        <div className="font-body text-xs font-medium text-text-secondary group-hover:text-text-primary truncate transition-colors">{event.name}</div>
        <div className="font-cond text-[10px] text-text-muted mt-0.5 truncate">
          {event.isLive ? <span className="text-crimson font-semibold tracking-wide">{event.phase}</span> : event.dateRange || event.phase}
        </div>
      </div>
      <span className="font-cond text-[9px] font-bold tracking-widest uppercase shrink-0" style={{ color: event.color, opacity: isUpcoming ? 0.5 : 1 }}>{event.region}</span>
...
export default function RightSidebar() {
  ...
  const displayMatches = [...liveMatches, ...upcomingMatches.slice(0, 10), ...completedMatches];

  const liveEvents    = EVENTS.filter((e) => e.isLive);
  const ongoingEvents = EVENTS.filter((e) => !e.isLive && !isUpcomingEvent(e));
    ...
    <aside className="bg-ink border-l border-ink-mid overflow-y-auto">
      ...
      {error && (
        <div className="mx-3 my-2 p-3 rounded-sm bg-crimson/5 border border-crimson/20">
          <p className="font-cond text-xs text-crimson/80 mb-2">Could not reach the lolesports API.</p>
          <button onClick={refresh} className="font-cond text-[10px] font-bold tracking-widest uppercase text-crimson/60 hover:text-crimson transition-colors">Retry →</button>
        </div>
      )}
      ...
      <div className="pb-2">
        {isLoading
          ...
          : displayMatches.length > 0
            ? displayMatches.map((m) => <MatchRow key={`${m.id}-${m.startTime}`} match={m} />)
            : !error && <p className="px-3 py-4 font-body text-xs text-text-muted text-center">No matches scheduled.</p>
        }
      </div>
      ...
      {liveEvents.length > 0 && (
        <>
          ...
          {liveEvents.map((e) => <EventRow key={e.id} event={e} />)}
        </>
      )}

      <SectionLabel>Ongoing Events</SectionLabel>
      <div className="pb-1">{ongoingEvents.map((e) => <EventRow key={e.id} event={e} />)}</div>

      <SectionLabel>Upcoming Events</SectionLabel>
      <div className="pb-6">{upcomingEvents.map((e) => <EventRow key={e.id} event={e} />)}</div>
    </aside>
  );
}
